"""
Mask R-CNN
Train on the Airbus Ship Detection dataset and submit to Kaggle Competition.

Copyright (c) 2018 Skymapglobal, Inc.
Licensed under the MIT License (see LICENSE for details)
Written by Giang Ngo
...
Usage: import the module (see Jupyter notebooks for examples), or run from
    ...
    python3 airbus_ship.py train --dataset=/path/to/airbus_ship/dataset --weights=coco

    # Resume training a model that you had trained earlier
    python3 airbus_ship.py train --dataset=/path/to/airbus_ship/dataset --weights=last

    # Train a new model starting from ImageNet weights
    python3 airbus_ship.py train --dataset=/path/to/airbus_ship/dataset --weights=imagenet

    # Apply ship detection to an image
    python3 airbus_ship.py detect --weights=/path/to/weights/file.h5 --image=<URL or path to file>
...
ROOT_DIR = os.path.abspath(r"E:\Shipdetection\scheduler_before_queue")
...
aoi_file = r"E:\Shipdetection\Run\oai\AOI_Bengkalis.geojson"
mask_file = r"E:\Shipdetection\Run\mask_folder\AOI_Bengkalis_LandMask.geojson"
...
"""
    ##################
        if này là để chỉ đoạn này ảnh predict nhỏ hơn cả kích thước cắt
    ##################
"""
if image_width <= chunk_size and image_height <= chunk_size:
    ...
    """
        ##############
            ảnh lớn hon kích thước cắt
        ##############
    """    
else:
    
    print("ảnh cắt lớn hơn")
...
#    print(padded_image)
    ...
    for cell in cells:
        ...
        results = model.detect([cell_image], verbose=1)
